...
def test_create_note_with_both_fields(tasks_url):
...
def test_create_note_with_only_name_field(tasks_url):
...
def test_create_note_empty_name(tasks_url):
    response = requests.post(tasks_url, json={"name": ""})
    assert response.status_code == 422


def test_create_note_extra_field(tasks_url):
...
def test_create_note_null_body(tasks_url):
    response = requests.post(tasks_url, json=None)
    assert response.status_code == 422


def test_get_notes(tasks_url, created_note):
...
def test_update_note(tasks_url, created_note):
...
def test_update_note_no_fields(tasks_url, created_note):
    note_id = created_note["id"]
...
def test_update_not_existing_note(tasks_url):
...
def test_delete_note(tasks_url, created_note):
    note_id = created_note["id"]
...
def test_delete_not_existing_note(tasks_url):
    resp = requests.delete(f"{tasks_url}/9999")
    assert resp.status_code == 404
    assert "detail" in resp.json()


def test_note_status_IN_PROGRESS(tasks_url):
...
def test_note_status_NO_TIMER(tasks_url):
...
def test_patch_can_swap_status_to_note(tasks_url, created_note):
    note_id = created_note["id"]
